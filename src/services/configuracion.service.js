...
export async function obtenerConfiguracion() {
  ...
  return {
    ...
    hora_apertura: row.hora_apertura || "08:00",
    hora_cierre: row.hora_cierre || "22:00",
    dias_atencion: row.dias_atencion || "Lunes a Domingo",
    abierto_manual: Number(row.abierto_manual ?? 1),
    costo_base_envio: parseFloat(row.costo_base_envio ?? 1.50),
    precio_por_km: parseFloat(row.precio_por_km ?? 0.50),
    distancia_maxima_km: parseFloat(row.distancia_maxima_km ?? 15.0),
    latitud_restaurante: parseFloat(row.latitud_restaurante ?? -0.180653),
    longitud_restaurante: parseFloat(row.longitud_restaurante ?? -78.467838),
